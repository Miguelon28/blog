	...
	<div class="container">
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM usuarios WHERE codigo='$usuario'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM usuarios WHERE codigo='$usuario'");
					if($delete){
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($filter){
					$sql =  "SELECT * FROM usuarios WHERE estado='$filter' ORDER BY codigo ASC";
				}else{
				...
				if(($sql) == 0){
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						<tr>
							...
							<td><a href="profile.php?nik='.$row['id'].'"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> '.$row['nombre'].'</a></td>
                            <td>'.$row['usuario'].'</td>
                            <td>'.$row['email'].'</td>
